...
on half of \texttt{train}, using the \texttt{caret} package. The
remaining half is left for validation\footnote{There is an actual CV
  framework being developed in the \texttt{R} package}.
...
easier than the original problem studied in section
\ref{sec:baseline}.
...
We run a full cross-validation below. The optimization is slow, but
exploiting both regression and matrix completion structure seems to
have led to a gain, though again these results are not directly
comparable to the baseline regressions from earlier (and that approach
might do better on this simpler data set). Also, we could speed up the
optimization using stochastic gradient descent, but the improvement
seemed marginal in preliminary experiments, and so we are sticking to
standard gradient descent here.
